"""
train.py — Fine-tunes DistilBERT for spam/ham classification using PyTorch.

DistilBERT (distilbert-base-multilingual-cased) is a distilled version of
bert-base-multilingual-cased. It is 40% smaller (66M vs 179M parameters),
60% faster at inference, and retains ~97% of mBERT's accuracy on most tasks.
Like mBERT it supports 104 languages, making it directly comparable.

Full pipeline (identical to mbert/train.py — only MODEL_NAME and SAVE_PATH differ):
...
Usage (run from project root):
  python -m src.distilbert_model.train          # WARNING — only problems
  python -m src.distilbert_model.train -v       # INFO    — milestones + accuracy
  python -m src.distilbert_model.train -vv      # DEBUG   — shapes, batch counts, LR
  python -m src.distilbert_model.train -vvv     # TRACE   — per-500-batch loss + LR

Training time (faster than mBERT due to fewer layers):
  CPU only : ~1–3 days for 2 epochs
  GPU      : ~2 minutes on Colab T4
  Recommend running on Google Colab — same workflow as mBERT.
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune DistilBERT for spam classification"
    )
